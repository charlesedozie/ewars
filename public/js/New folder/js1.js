// JavaScript Document
//var turnServer = "http://turn.dredgeresources.com";
//var ioServer = "https://socket.1and1chat.com";  
...
let depts = '<option value="0" selected> -- Choose Department --</option>';	
...
if(socket.connect()){
socket.emit('get-all', {msg: "msg", sessionId:localStorage.getItem("sessionId"), sectionId:-1});
//document.getElementById("sendVisitorMsg").removeAttribute("disabled");
//document.getElementById("chatMsg").removeAttribute("disabled");
}
}
}
...
//document.getElementById("faicons").setAttribute("class", "card-footer p-1 hideElem") ;
clearInterval(startRunApp);
document.getElementById("offlineInterface").style.display = "none"
document.getElementById("onlineInterface").style.display = "none"
document.getElementById("controlBtnCon").style.display = "none"
document.getElementById("showChatEnd").style.display = "block"
//}
}// end reset chat
socket.on("disconnect", () => {  resetChat() ;console.log(socket.connected); });
...
//socket.on("connect", () => {
  //log(localStorage.getItem("sessionID"))
  //const content = { "sessionID":localStorage.getItem("sessionID"), "domain":"home.com" }
  //socket.auth = content;
  
 // socket.emit('audio-offer', {sdp:"Charles reo", to:"target", from: localStorage.getItem("sessionID")})
...
//document.getElementById("sendVisitorMsg").removeAttribute("disabled");
document.getElementById("showAgtMsg").removeAttribute("class");
document.getElementById("showAgtJoined").style.display = "none"
//document.getElementById("faicons").setAttribute("class", "card-footer p-1") ;
textMsg = `${data.msg}`;
...
if(data.msg.length > 0){document.getElementById("showAgtMsg").removeAttribute("class");
document.getElementById("showAgtJoined").style.display = "none"}
//document.getElementById("sendVisitorMsg").removeAttribute("disabled");
//document.getElementById("chatMsg").removeAttribute("disabled");
//document.getElementById("faicons").setAttribute("class", "card-footer p-1") ; }					  
...
console.log("reconnect get all");
console.log("sessionid "+data.sessionId);
console.log("usertype "+data.userType);
});


//console.log(localStorage.getItem("clientsocketid"))
...
function appDef(){<div class="">
defCon = document.createElement("section"); 
defCon.setAttribute("class", "defCon"); 
defCon.setAttribute("id", "defCon"); 

defCon1 = document.createElement("section"); 
defCon1.setAttribute("class", "row d-flex flex-row-reverse"); 
defCon1.setAttribute("id", "defCon1"); 

defCon.append(defCon1);

defChatCon = document.createElement("section"); 
defChatCon.setAttribute("class", "defChatCon"); 
defChatCon.setAttribute("id", "defChatCon"); 

defCon1.append(defChatCon);
	}
	
	
var startRunApp = null;
window.addEventListener('load', (event) => { startRunApp = setInterval(runApp, 1000); appDef(); stopFormSubmit('form1', 'form2'); reConnect();});
