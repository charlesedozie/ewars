...
const userVerify = await Db.checkValue("SELECT id FROM user WHERE status = 1 AND uuid="+con.escape(authBody[1])+" AND domain="+con.escape(authBody[2]));
...
const user = await Db.query ("SELECT id, domain, typeid, username, usernameiv, status, uuid, verify FROM user");
...
const { username, domain, otp, password, confirmpassword, frmMode } = args;
...
if (!username || username.trim() === '' || username === "undefined" || !domain || domain.trim() === '' || domain === "undefined" || !otp || otp.trim() === '' || otp === "undefined" || !password || password.trim() === '' || password === "undefined" || !confirmpassword || confirmpassword.trim() === '' || confirmpassword === "undefined") { result[0].status = 0; result[0].msg = result[0].msg+"Error! Incorrect entry";  } 
...
if(result[0].status === 0) {return result; }
  ...
  const user = await Db.query ("SELECT id, domain, typeid, username, usernameiv, status, uuid FROM user");
  if(typeof user === 'object' && user !== null && user.length > 0){      
    user.every(async function(arrayItem, index) {
      const decrypt = Utils.decrypt ({iv: arrayItem.usernameiv, content: arrayItem.username});
      if (decrypt === username.toLowerCase()) { 
        loggedIn.found=1; otpUserId=arrayItem.id; userDomain=arrayItem.domain;
      }
    });
  }
