...
let status = 2; 
let total = await Db.query("SELECT status, id FROM user WHERE status = 1 AND id = " + con.escape(userId));
if(total.length > 0){ // process for user found
if(parseInt(total[0].id) === 1){status = 1; return parseInt(status);} // root access
...
console.log(id);
let orgname = await Db.query("SELECT fullname FROM useprofile WHERE status = 1 AND userid = "+con.escape(parseInt(id))+" ORDER BY id DESC LIMIT 0, 1");    
console.log(orgname);
if(orgname.length > 0){ return orgname[0].fullname; }
return '';
}
